...
--   Build order-level revenue (price summed per order)
...
--   Aggregate those order totals by calendar month
...
--
--  Revenue for Jan-Sep only (across 2017 & 2018)
...
--   Build monthly order totals (revenue per order + month tag)
...
--   Compute average order value per month
...
--  Join items → products to get the category name
...
--  Revenue by category for Jan–Sep 2017 & 2018
-- Multi CTE query
...
--  Pivot into 2017 vs 2018 and compute growth %
...
--   Compute revenue & units per category
...
--   Derive average price per item (AOV proxy at item level)
...
-- Filter categories below median units but above median avg-price
...
**Revenue & Growth** – monthly GMV trend, Jan-Sep YoY growth (-› +99 %), and AOV stability showed that topline expansion is driven by sheer order volume, not bigger baskets.  
**Product & Category Performance** – top-10 categories revealed where 60 % of cash sits; fast-growing niches (home appliances +5 572 % YoY) flagged emerging demand; a high-AOV/low-volume list exposed premium segments like PCs (R$ 1 099 per item).  
**Geographic Insights** – city-level revenue proved Rio de Janeiro and São Paulo dominate GMV; delivery-time heat-map exposed interior towns waiting 60-148 days; order- vs-revenue share showed Rio buyers spend 3× the national average while São Paulo orders are high but lower-margin.
